...
double getReloadTime(void) {
    ...
    printf("Battleship reload time (T_B, seconds between shots):\n");
    ...
    if (choice == 1) {
        ...
        t = randomDouble(0.2, 3.0);
        printf("Randomly generated T_B = %.2f\n", t);
    }
    return t;
}
...
void initializeBattleship(Battleship *b, double canvasSize) {
    ...
    if (choice == 1) {
        ...
        b->maxVelocity = randomDouble(200.0, 800.0);
        printf("Randomly generated Vmax_B = %.2f\n", b->maxVelocity);
    }

    b->isDestroyed = 0;
    b->cumulativeDamage = 0.0;
    b->reloadTime = 0.0;
}
